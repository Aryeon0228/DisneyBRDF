...
export function initRoomLab(showRoom){
 const $=id=>document.getElementById(id);let state=roomDefaults(),queued=false;
 ...
 function update(){
  ...
  if(!queued){queued=true;requestAnimationFrame(()=>{queued=false;renderRoomPair($('room-left-canvas'),$('room-right-canvas'),state);});}
